    ...
    <form class="row container p-3 needs-validation" method="post" action="{{$route}}" novalidate>
        ...
        @foreach( $fields as $field)
                    ...
                    <div class="row mt-4">
                        <select class="col border rounded-2   p-2 text-reset" name="{{$field['name']}}" id="input_{{$field['name']}}" required>
        ...
        @if($target == 'Module')
            ...
            <div class="row mt-4"><h5>Gestion des Devoirs du Module</h5></div>
            <div class="row">
                <p class="text-reset">
                    Veuillez ajouter les devoirs de votre module.
                </p>
            </div>
            <div class="row"><hr class="mt-2 dropdown-divider"></div>
            <div class="row" id="devoirs">
                ...
                <button type="button" id="modal-trigger" class="btn btn-primary" data-toggle="modal" data-target="#popup" hidden>
                </button>
                ...
                @endif
            @endif
            <div class="row mt-4 mb-4"><hr class="dropdown-divider"></div>
            <div class="mt-4 d-flex justify-content-end">
                <button class="btn btn-success"><h6>{{(!isset($etudiant) && !isset($user) && !isset($module))?"Crée  $target":"Modifier  $target"}}</h6></button>
            </div>
     </form>
